...
public class DeleteFileHandler implements CommandHandler {
    private static final String INVALID_DELETE_COMMAND_FORMAT =
    ...
    private static final String SERVER_PROBLEM_MESSAGE =
        "Server problem 500";
    private static final String SUCCESSFUL_MESSAGE = "Successful deletion!";
    private static final int DELETE_ARGUMENT_COUNT = 1;
    ...
    public void handle(ResponseWriter writer, CommandContext ctx) {
        ...
        if (args.size() != DELETE_ARGUMENT_COUNT) {
            writer.write(INVALID_DELETE_COMMAND_FORMAT);
            return;
        }
        ...
        try {
            ctx.services().userDirectoryService()
                .delete(loggedUsername, args.get(ARGUMENTS_FILENAME_IND));
